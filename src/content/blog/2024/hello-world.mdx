...
I decided I wanted to make a blog after reading a number of independent blogs
and finding them inspiring, it got me thinking about projects I have worked on
or built and I felt a bit sad that there was no way to see some of my oldest
code and projects as they are not logged anywhere, this tends to be at odds with
my normal desire to document things and share any and all accrued knowledge.
...
deliver _something_. I wanted to try and keep things as simple as possible while
having a solid non-limiting foundation that I can easily extend.
...
[MDX](https://mdxjs.com/). To me it seemed perfect for writing blog posts as my
markdown posts can be enriched by sticking components into them like the
following: <Grower client:load>CLICK ME!!!</Grower>.

Knowing I would prefer to use an SSG all I needed to do was find one that worked
like I wanted. I had heard good things about [11ty](https://www.11ty.dev/), so
tried it out but quickly found it a little limiting for my desired use cases, as
it did not support JSX for templating. I started making an 11ty plugin for this
behaviour but it felt like I was fighting 11ty to get what I wanted, so I
...
[JSX](https://www.11ty.dev/docs/languages/jsx/)). When looking at different SSGs
I came across a framework called [Astro](https://astro.build/) which from the
sales pitch seemed suspiciously perfect, so I built a little hello world project
and to my surprise everything I wanted was supported with minimal work!! With
pretty much just adding 1st party dependencies, I could create blog posts in
MDX, create templates in JSX AND generate a fully static site with components
that could be loaded at runtime using the "islands architecture".
...
blogpost's quality, so started working on other projects then I broke my elbow
and found typing hard but after being inspired by
[TodePonds](https://mas.to/@TodePond) mastodon posts to
